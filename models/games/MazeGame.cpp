...
void MazeGame::initHypotheses() {
	...
	LOG_DEBUG("Initialised Maze Game.")
}

void MazeGame::initIMs() {
	//get alive units
...
GameReplayCommand* MazeGame::parseReplayCommand(std::ifstream& ifs, double timeStamp, std::string command) {
	if (command == "TCHG") {
...
void MazeGame::applyReplayCommand(GameReplayCommand* c) {
...
void MazeGame::selectTarget(Unit* predator, std::vector<Unit*>& prey) {
