...
EvasionGame::EvasionGame() : Game() {
	float r45 = sin(M_PI/4);
	mOffsets.push_back(osg::Vec3(0,1,0));
	mOffsets.push_back(osg::Vec3(r45,r45,0));
	mOffsets.push_back(osg::Vec3(1,0,0));
	mOffsets.push_back(osg::Vec3(r45,-r45,0));
	mOffsets.push_back(osg::Vec3(0,-1,0));
	mOffsets.push_back(osg::Vec3(-r45,-r45,0));
	mOffsets.push_back(osg::Vec3(-1,0,0));
...
EvasionGame::~EvasionGame() {
}

void EvasionGame::initHypotheses() {
	...
	for (std::vector<UnitGroup*>::iterator it = predatorGroups.begin(); it != predatorGroups.end(); it++) {
		Hypothesis *h = new TargetDirectionHypothesis(preyGroups, *it);
	...
	LOG_DEBUG("Initialised Evasion Game.")
}

void EvasionGame::initIMs() {
	//get alive units
	...
	for (std::vector<Unit*>::iterator it = predators.begin(); it != predators.end(); it++) {
		InverseModelTrack* im = new InverseModelTrack(*it);
		mPredatorModels[*it] = im;
		if (!isReplay()) theApp->addInverseModel(im);
	}
}

GameReplayCommand* EvasionGame::parseReplayCommand(std::ifstream& ifs, double timeStamp, std::string command) {
	if (command == "TCHG") {
...
void EvasionGame::applyReplayCommand(GameReplayCommand* c) {
...
void EvasionGame::updateGame() {
	...
	for (std::vector<Unit*>::iterator it = predators.begin(); it != predators.end(); it++) {
		...
		if (!isReplay() && ((mPredatorModels[*it] && mPredatorModels[*it]->getTarget() == NULL) || (rand() % 10000 < 1))) {
			//randomly select new target & IM (roughly every 10000 frames)
...
void EvasionGame::selectTarget(Unit* predator, std::vector<Unit*>& prey) {
